...
class GoogleCalendar:
    ...
    def __init__(self, scopes: list, creds_path: str) -> None:
        self._scopes = scopes
    ...
    def _get_events_from_list(self, schedule: list) -> list:
        events = []
        for lesson in schedule:
            if lesson["subj"] != "":
                event = self._create_calendar_json(
                    lesson["subj"], lesson["loc"], lesson["start"], lesson["end"]
                )
                events.append(event)
        return events
    ...
    def _create_calendar_json(self, subj: str, loc: str, start: str, end: str) -> dict:
        event = {
            "summary": subj,
            "location": loc,
            "description": "",
    ...
    def _get_calendar_id(self, group_name: str) -> str:
    ...
    async def upload_schedule(
        self, schedule: list, group_name: str, calendar_id: str = ""
    ) -> list:
        ...
        for event in events:
            ...
            links.append(event_upl.get("htmlLink"))
        return links

    async def update_schedule(self, schedule: list, group_name: str):
        dates = [self._google_dt(lesson["start"])[:10] for lesson in schedule]
        mon, sat = min(dates), max(dates)
        calendar_id = self._get_calendar_id(group_name)
        calendar_events = (
            self.service.events()
            .list(calendarId=calendar_id, pageToken=None)
            .execute()["items"]
        )
