...
from config import SCOPES, FORM_ID
...
class GoogleForms:
    ...
    def _get_form_responses(self) -> list:
        return self.service.forms().responses().list(
            formId=self.__form_id
    ...
    def _get_latest_responses(self) -> list:
        ...
        todays_responses = [
            resp for resp in self._get_form_responses() if
            resp['lastSubmittedTime'].split('T')[0] == today
        ]
    ...
    def get_responces(self) -> list:
        ...
        return [
            self._parse_response(resp) for resp in responces
        ]
