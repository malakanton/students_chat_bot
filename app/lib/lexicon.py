...
🔹Здесь ты можешь задавать мне любые вопросы, я постараюсь на них ответить.

🔹Чтобы посмотреть расписание нажми на /schedule.

🔹Могу уведомлять тебя за несколько минут до предстоящего занятия или присылать по утрам расписание на день, 
для настроек выбери команду /notifications.

🔹Для доступа к учебным материалам вызови команду /library и я пришлю любой файл из тех что у меня есть.
...
CONTACTS = """
Для обратной связи напиши здесь свое сообщение и обязательно начние его с хештега #support.
"""
...
MORNING_SCHEDULE = """Привет! Сегодня занятия в колледже:\n\n"""
...
NOTIFICATIONS_SET_TEXT = """
За несколько минут до занятия я буду присылать сообщение с деталями занятия и ссылкой, если она у меня есть.

Уведомления работают только в будние дни.

Выбери за сколько минут присылать напоминалку:
"""

NOTIFICATIONS_ON_BEGIN = """
Уведомления работают только по будним дням.

Для изменения настроек выбери тип уведомлений которые хочешь изменить:
"""

PUSH_TIME_MESSAGE = """Введи время в формате HH:MM"""

INVALID_PUSH_TIME = """Неверный формат времени, попробуй еще раз"""

PUSHING_OFF = """
Я могу присылать тебе расписание на день (если в этот день есть занятия) в выбранное тобой время.

Нажми OFF если уведомления не нужны.
"""

PUSHING_ON = """
В дни когда есть занятия я буду присылать тебе расписание на 
день в {}, чтобы легче было спланировать свой день и не опоздать на пары.
"""

NOTIFICATIONS_DIALOG_END = """
Чтобы изменить настройки, снова нажми /notifications
"""

NOTIFICATIONS_CHANGE = """Успешно обновил настройки уведомлений:\n\n"""

NOTIF_FLAG = [
    '🔴Напоминалки о парах *выключены*\n',
    '🟢Напоминалки перед парой - *за {} минут*\n'
]

NOTIF_PUSH = [
    '🔴Ежедневные оповещения *выключены*\n',
