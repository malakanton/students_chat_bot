...
📨*/contacts* - написать в обратную связь
"""


DESCRIPTION = """
Привет, я бот для студентов КТ МТУСИ очно-заочного отделения.

Я помогу быстро ознакомиться с расписнием (V.1).
Сообщу о загрузке нового расписания (V.1.1).

Чуть позднее я буду собирать и помогу находить материалы по учебным предметам (тексты лекций и шаблоны конспектов, домашних заданий) когда их кто-то загрузит (V.2).
"""
